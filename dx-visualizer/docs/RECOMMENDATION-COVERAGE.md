...
| `bgp-route-limit` | API-supported | ≤100 prefixes on-prem → AWS on private/transit VIFs | Exact accepted-route count from `ListVirtualInterfaceRoutes` once routes have been fetched; otherwise CloudWatch `AWS/DX` `VirtualInterfaceBgpPrefixesAccepted` (IPv4 + IPv6 summed). Escalates by observed prefix count: **Critical** at 100+ (per-session hard limit — session at risk of teardown), **Warning** within 20 of the limit (80+), **Info** within limits or when neither source has data (e.g. brand-new VIF) |
...
| `vif-route-symmetry` | API-supported (BGP routes) / Guidance | Summarize prefixes; keep VIF routing symmetric | With route data, flags prefixes already covered by a less-specific prefix on the same VIF — each wastes one of the 100 accepted prefixes (**Warning**). Falls back to guidance otherwise |
