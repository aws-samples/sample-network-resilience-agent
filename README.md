...
| **DNS & routing** | Route 53 and prefix lists are not displayed (BGP ASN is shown on relevant nodes). TGW and VPC route tables are viewable via inline panels on each TGW / VPC node. Per-VIF BGP routes and a cross-VIF prefix comparison per DX Gateway are viewable in live mode — these need `directconnect:ListVirtualInterfaceRoutes`, which the AWS managed `ReadOnlyAccess` policy does **not** grant (see [SETUP.md §4.2](dx-visualizer/docs/SETUP.md#42-required-iam-permissions)). |
